...
Improvements made in this map include:
1. Convert geographic data using st_centroid() for point representation
2. Use colorFactor() with 'viridis' palette for consistent sector coloring
3. Compute centroids outside reactive context for better performance
4. Replaced polygon geometries with circle markers to solve visibility issues with small areas
...
### 3.3 Adding Tabs for Analysis & Data Table

- Using polygons instead of circled markers
- Adding a tab for Analysis with barplots and pie charts (allowing filter selection)
...
server <- function(input, output, session) {
  ...
  filtered_data <- reactive({
    ...
    data <- data %>% 
      filter(
        Amount..Constant.USD.2021./1e6 >= input$amount_filter[1],
  ...
  output$map <- renderLeaflet({
    ...
    # Create color palette based on Sector Name
    pal <- colorFactor("viridis", domain = combined_geojson$Sector.Name)
    
    leaflet(data) %>%
      addProviderTiles("CartoDB.Positron") %>%
      addPolygons(
        fillColor = ~pal(Sector.Name),
        color = ~pal(Sector.Name), # Match border color to sector for better visual alignment
        weight = 1,
        opacity = 1,
        fillOpacity = 0.7,
        ...
        popup = ~paste(
          "<b>Title:</b>", Title, "<br>",
          "<b>Investment:</b> $", 
          formatC(Amount..Constant.USD.2021./1e6, format = "f", digits = 2), "M<br>",
  ...
  output$project_table <- renderDT({
    filtered_data() %>%
      st_drop_geometry() %>%
      select(
        `Title`, `Recipient Region`, `Sector.Name`, `Status`,
